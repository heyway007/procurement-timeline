...
export function HolidayManager({ initialYear = new Date().getFullYear(), initialHolidays, previewMutation = apiPreview }: HolidayManagerProps) {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-4 py-8 sm:px-6">
    ...
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold text-indigo-700">ปฏิทินหน่วยงาน</p><h1 className="mt-1 text-3xl font-semibold">จัดการวันหยุดราชการ</h1><p className="mt-2 text-sm text-slate-600">วันหยุดในรายการจะไม่นำมานับใน Timeline ทุกโครงการ</p></div><button type="button" onClick={() => setAdding(true)} className="rounded-xl bg-indigo-700 px-5 py-3 font-semibold text-white">เพิ่มวันหยุด</button></header>
    ...
    <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">{holidays.length === 0 ? <p className="p-8 text-center text-slate-600">ยังไม่มีข้อมูลวันหยุดในปีนี้</p> : holidays.map((holiday) => <div key={holiday.id} className="grid gap-2 border-b border-slate-100 px-5 py-4 sm:grid-cols-[10rem_1fr]"><p className="font-semibold text-indigo-700">{formatThaiDate(holiday.date)}</p><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{holiday.name}</p><span className="rounded-full bg-indigo-50 px-2 py-1 text-xs text-indigo-800">{holiday.scope === "BANGKOK" ? "กรุงเทพมหานคร" : "ทั่วประเทศ"}</span><span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">{holiday.origin === "OFFICIAL_SYNC" ? "ราชการ" : "เพิ่มเอง"}</span></div><p className="mt-1 text-sm text-slate-500">{holiday.sourceNote}</p>{holiday.officialSourceUrl ? <a href={holiday.officialSourceUrl} target="_blank" rel="noreferrer" className="mt-1 inline-block text-xs text-indigo-700 underline">แหล่งข้อมูล</a> : null}</div></div>)}</div>
