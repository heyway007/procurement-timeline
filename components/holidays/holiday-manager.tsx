...
export function HolidayManager({ initialYear = new Date().getFullYear(), initialHolidays, previewMutation = apiPreview }: HolidayManagerProps) {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl overflow-x-hidden px-4 py-6 sm:px-6 sm:py-8">
    <header className="flex flex-col gap-5 border-b border-slate-200 pb-6 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-sm font-semibold text-indigo-700">ปฏิทินหน่วยงาน</p><h1 className="mt-1 text-2xl font-semibold sm:text-3xl">จัดการวันหยุดราชการ</h1><p className="mt-2 text-sm text-slate-600">วันหยุดในรายการจะไม่นำมานับใน Timeline ทุกโครงการ</p></div><Link href="/" className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:border-indigo-200 hover:text-indigo-700"><FontAwesomeIcon icon={faArrowLeft} aria-hidden="true" className="h-4 w-4 shrink-0" /><span>กลับหน้าโครงการ</span></Link></header>
    <section className="mt-6 grid gap-4 md:grid-cols-[12rem_minmax(0,1fr)_auto] md:items-start"><label className="block text-sm font-medium">ปี ค.ศ.<input type="number" value={year} onChange={(event) => setYear(Number(event.target.value))} className="mt-2 min-h-11 w-full rounded-xl border border-slate-300 px-3" /></label>{sync ? <div className={`min-w-0 rounded-xl px-4 py-3 text-sm ${sync.status === "FRESH" ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-900"}`}><p className="font-semibold">{sync.status === "FRESH" ? "ข้อมูลวันหยุดราชการเป็นปัจจุบัน" : sync.status === "CACHED" ? "กำลังใช้ข้อมูลราชการที่บันทึกไว้" : "ยังดึงข้อมูลราชการไม่ได้"}</p>{sync.lastSuccessfulSyncAt ? <p className="mt-1">อัปเดตสำเร็จล่าสุด {new Date(sync.lastSuccessfulSyncAt).toLocaleString("th-TH")}</p> : null}{sync.sourceUrl ? <a className="mt-1 inline-block underline" href={sync.sourceUrl} target="_blank" rel="noreferrer">เปิดแหล่งข้อมูลราชการ</a> : null}</div> : null}<button type="button" onClick={() => setAdding(true)} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-indigo-700 px-5 py-3 font-semibold text-white hover:bg-indigo-800 md:col-start-3 md:w-auto"><FontAwesomeIcon icon={faCalendarPlus} aria-hidden="true" />เพิ่มวันหยุด</button></section>
    ...
    {preview ? <div className="fixed inset-0 z-[60] grid place-items-center overflow-y-auto bg-slate-950/50 p-3 sm:p-4"><section role="dialog" className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl sm:p-6"><h2 className="text-xl font-semibold">ผลกระทบต่อ Timeline</h2><p className="mt-3 text-sm text-slate-600">โครงการที่อาจถูกคำนวณใหม่ {preview.affectedProjects.length} รายการ</p><ul className="mt-4 list-disc space-y-1 pl-5">{preview.affectedProjects.map((project) => <li className="break-words" key={project.id}>{formatProjectName(project.name)}</li>)}</ul><div className="mt-6 grid gap-3 sm:flex sm:justify-end"><button type="button" onClick={() => setPreview(undefined)} className="min-h-11 rounded-xl border px-4 py-2">ย้อนกลับ</button><button type="button" onClick={() => void confirm()} className="min-h-11 rounded-xl bg-indigo-700 px-4 py-2 font-semibold text-white">ยืนยันและคำนวณใหม่</button></div></section></div> : null}
  </main>;
}
