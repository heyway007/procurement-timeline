    ...
    <main className="mx-auto min-h-dvh max-w-7xl overflow-x-clip px-4 py-6 pb-64 sm:px-6 sm:py-8 sm:pb-8 lg:px-8">
      ...
      <section aria-label="ตัวกรองโครงการ" className="my-6 grid gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:my-7 sm:grid-cols-3">
        <label className="min-w-0 text-sm font-medium text-slate-700">
          ค้นหาโครงการ
          <input className="mt-2 min-h-11 w-full min-w-0 max-w-full rounded-xl border border-slate-300 px-3 text-base" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ชื่อโครงการหรือผู้รับผิดชอบ" />
        </label>
        <label className="min-w-0 text-sm font-medium text-slate-700">
          ช่วงวันที่เริ่ม
